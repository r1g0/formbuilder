...
const INITIAL_STATE = {
  ...
  schema: {
    ...
    description: "Agregue una descripción de la encuesta.",
    properties: {}
  },
  uiSchema: {
    "ui:order": []
...
function addField(state, field) {
  ...
  const name = `Pregunta ${state.currentIndex}`;
  ...
  state.schema.properties[_slug].index = state.uiSchema["ui:order"].length;
